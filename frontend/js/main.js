...
import { loadDeliveryPartners, shipProduct } from "./distributor.js";
...
import { initQrFeatures } from "./qr.js";
import { loadPendingRecallRequests, requestRecall, reviewRecall } from "./recall.js";
import { initAutomaticBatchCodes, loadBatchOptions } from "./batches.js";
...
const formHandlers = {
  ...
  "harvest-form": (data) => harvestProduct(data.get("batchCode"), data.get("note")),
  "create-processed-product-form": (data) => createProcessedProduct(data.get("sourceBatchCode"), data.get("outputBatchCode"), data.get("name"), data.get("description"), data.get("materialQuantity"), data.get("quantity"), data.get("unit"), data.get("expiryDate"), data.get("note")),
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  Object.entries(formHandlers).forEach(([id, handler]) => {
    ...
    form?.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        if (success) { form.reset(); await Promise.all([loadDashboard(), loadPendingRecallRequests(), loadBatchOptions()]); }
  ...
  onWalletChanged(loadProtectedData);
